...
def chat_section(go_chat):
  while(go_chat):
    print("-"*48)
    user_query=input(f"Enter your qsn  dear {user_name}😎:")
    chat_histroy.append(HumanMessage(content=user_query))
    print(user_query)
    ...
    #   chain= template | modell
      ...
      chat_histroy.append(AIMessage(content=assistant_response))
      print(f"\n🗣️ Assistant:\n{assistant_response}")
      print("-"*48)
      
  print(f"\nThanks For Interacting With Our Chat Model🎖️🎖️🎖️🎖️")

chat_section(go_chat)
